...
class Snake:
    ...
    binary = None       # The image in binary 
        ...
        for i in range( 0, len( self.points ) ):
            ...
            for dx in range( -HALF_KERNAL_SIZE, HALF_KERNAL_SIZE ):
                for dy in range( -HALF_KERNAL_SIZE, HALF_KERNAL_SIZE ):
                    ...
                    e_term[ dx + HALF_KERNAL_SIZE ][ dy +HALF_KERNAL_SIZE ] = self.f_term( p, prev, next )
            ...
            Total_Energies =(self.alpha * e_uniformity) + (self.beta * e_curvature) + (self.w_line * e_line) + (self.w_edge * e_edge) + (self.w_term * e_term) + (self.delta * e_conf)
